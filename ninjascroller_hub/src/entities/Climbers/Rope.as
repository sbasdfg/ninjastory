package entities.Climbers 
{
	...
	public class Rope extends Box2DPhysicsObject 
	{
		...
		override public function handlePreSolve(contact:b2Contact, oldManifold:b2Manifold):void
		{
			...
			{
				var closeRope:Boolean = false;
				//if (hero.y > (this.y + this._height) && (hero.y + hero.height) < this.y) closeRope = true;
				//trace (hero.y, (this.y + this._height), (hero.y + hero.height), this.y)
				if (!ropeAdded && !hero.body.GetJointList() && hero._canSwingAgain)
				{
					...
					if (targetJointIndex < 1) return;
					else if (targetJointIndex > _vecBodyRope.length - 1) return;
					//targetJointIndex = _vecBodyRope.length - 1;
